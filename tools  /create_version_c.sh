...
VKDT_SRC_DIR=$(dirname "$0")
VKDT_SRC_DIR=$(cd "$VKDT_SRC_DIR/../" && pwd -P)
...
LAST_COMMIT_YEAR=$("${VKDT_SRC_DIR}/tools/get_last_commit_year.sh")
...
# when changing format, you must also update tools/get_git_version_string.sh !!!
  {
    ...
    echo "  const char vkdt_package_version[] = \"${NEW_VERSION}\";"
    echo "  const char vkdt_package_string[] = PACKAGE_NAME \" ${NEW_VERSION}\";"
    echo "  const char vkdt_last_commit_year[] = \"${LAST_COMMIT_YEAR}\";"
    echo "#else"
    echo "  #define VKDT_MAJOR ${MAJOR_VERSION}"
    echo "  #define VKDT_MINOR ${MINOR_VERSION}"
    echo "  #define VKDT_PATCH ${PATCH_VERSION}"
    echo "  #define VKDT_N_COMMITS ${N_COMMITS}"
